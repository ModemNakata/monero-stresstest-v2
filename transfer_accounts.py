#!/usr/bin/env python3
"""
Monero Account Transfer
Transfer funds from one account to subaddresses of another account
"""
...
class MoneroTransfer:
    ...
    def transfer(self, from_account, destinations, amount_per_dest):
        """Send transfer"""
        dest_list = []
    ...
    def transfer_between_accounts(self, from_account, to_account, amount_per_addr):
        ...
        # Send transfer
        print(f"\n⏳ Sending transfer...")
        result = self.transfer(from_account, destination_addrs, amount_per_addr)
        ...
        print(f"  Total Cost: {self.format_xmr(amount + fee):.12f} XMR")
...
def main():
    parser = argparse.ArgumentParser(
        description="Transfer funds between Monero accounts",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Transfer 0.1 XMR from account 0 to first 16 subaddresses of account 1
  python3 transfer_accounts.py 0 1 100000000000

  # Transfer 0.001 XMR to each address
  python3 transfer_accounts.py 0 2 1000000000

  # Custom wallet URL
  python3 transfer_accounts.py --wallet-url http://localhost:28088 0 1 500000000000
        """
    )
    ...
    transfer = MoneroTransfer(args.wallet_url)
